...
| 2026-04-19 | Blog post card HUD aesthetic redesign | Redesigned `PostEntry.tsx` with Framer Motion hover lift (`y: -4`), overlapping category badge with blinking cursor, corner SVG L-bracket, darker `bg-[#1c1c22]` body, `border-t border-accent` top anchor, scan-line texture on image, `line-clamp-2/3` truncation, accent glow shadow on hover, and redesigned byline with date pushed right. Removed bottom sliding accent line. |
